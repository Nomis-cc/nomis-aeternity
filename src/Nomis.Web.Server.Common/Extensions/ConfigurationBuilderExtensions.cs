...
namespace Nomis.Web.Server.Common.Extensions
{
    ...
    public static class ConfigurationBuilderExtensions
    {
        ...
        public static IConfigurationBuilder AddJsonConfigs(this IConfigurationBuilder manager)
        {
            ...
            return manager
                .AddJsonFile(Path.Combine(ConfigsDirectory, $"{nameof(AeternityExplorerSettings).ToLower()}.json"), false, true)
                .AddJsonFile(Path.Combine(ConfigsDirectory, $"{nameof(AeternityExplorerSettings).ToLower()}.{env}.json"), true, true);
        }
    }
}